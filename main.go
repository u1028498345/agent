...
// read from @minicap and send jpeg raw data to channel   读取minicap返回的数据并存到管道中
...
func getAddr() string {
	...
	if err != nil {
		fmt.Println("获取本机IP地址失败")
	}

	for _, value := range addrs {
		if ipnet, ok := value.(*net.IPNet); ok && !ipnet.IP.IsLoopback() {
			ip := ipnet.IP
			list_temp := strings.Split(ip.String(), ".")
			if ip.To4() != nil && list_temp[len(list_temp)-1] != "1" {
...
func main() {
	...
	fTunnelServer := cmdServer.Flag("server", "server url").Default(getAddr() + "8000").Short('t').String()
